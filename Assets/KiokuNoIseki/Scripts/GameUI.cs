...
■ 目的（勝利条件）
・相手のHPを0にする（通常勝利）。ただし相手の記憶領域にカードがある間はHPに通らない（下記）。
・記憶領域にカードを3枚そろえる（古き盟約勝利）。
・遺構デッキが尽きたとき、記憶領域の枚数が多い方が勝利（枯渇勝利）。

■ 記憶領域＝盾 兼 勝利条件（重要）
・自分の守護者が刻印3に達すると、自ターン終了時に『記憶領域』へ昇華（場から退場）する。
・記憶領域のカードは『残響＝防御力＋刻印』の盾になる。相手の本体直接攻撃は、まず記憶領域の残響を削り、砕けたカードは遺構デッキへ戻る。
・記憶領域が空のときだけ、攻撃がHPに通る。
・記憶領域に3枚そろえば古き盟約勝利。盾として守りつつ勝ち筋にもなる。
...
■ ターンの流れ（5フェイズ）
0. 生存刻印：前の自分の番から場に生き残っている守護者は刻印+1（攻防+1）。
...
■ 刻印・転生（共有デッキの肝）
・刻印は『生存』（毎ターン+1）と『破壊されての転生』（+1）で貯まる。刻印1つにつき攻撃/防御+1。
・破壊された守護者は墓地ではなく遺構デッキにシャッフルで戻る（刻印は保持）。
・真名・技はカード固有なので、相手が掘り当てれば相手がその技を使える。
・刻印3に達した自分の守護者は、自ターン終了時に記憶領域へ昇華する（上の「記憶領域＝盾」参照）。

■ 操作方法
・手札カードをクリック：プレイ（対象は自動選択）。
・自分の守護者をクリック：選択→『技』『攻撃』。
・攻撃：相手守護者をクリックで対象指定、または本体を直接攻撃（相手の記憶領域があればそちらが先に受ける）。
・『刻む』→捧げる手札をクリック。
・『ターン終了』でAIの番へ。";
