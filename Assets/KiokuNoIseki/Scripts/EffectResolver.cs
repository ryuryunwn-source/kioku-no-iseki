...
{
    // 技・想起術の効果を解決する。対象は15-4の「デフォルト対象」を自動採用する
    // （v1ではタップで自己解決させ、操作を単純化する）。
    public static class EffectResolver
    {
        static void Log(GameEngine g, string s) => g.OnLog?.Invoke("  " + s);

        // chosenTarget: 想起術などでプレイヤーが手動指定した対象（nullなら15-4のデフォルト対象を自動採用）
            ...
            CardInstance source, EffectId eff, int mag, CardInstance chosenTarget = null)
        {
            switch (eff)
            {
                ...
                case EffectId.ExtraExcavate:
                    ...
                    // 即時に追加発掘（行動フェイズ中なので手札に直接加える）
                    {
                        var c = g.deck.DrawTop();
                        if (c != null) { c.turnsInHand = 0; self.hand.Add(c); self.extraExcavatePending -= 1; Log(g, "追加で1枚発掘。"); }
                    }
                    break;
                ...
                case EffectId.ResetWeathering:
                {
                    ...
                    if (t != null) { t.weatheringCounter = 0; t.turnsInHand = 0; Log(g, $"「{t.definition.trueName}」の風化をリセット。"); }
                    break;
                }
                case EffectId.ReturnMemoryToDeck:
                {
                    var t = self.memoryZone.FirstOrDefault();
                    if (t != null) { self.memoryZone.Remove(t); g.deck.ReturnToRandomPosition(t); Log(g, $"記憶領域の「{t.definition.trueName}」を遺構へ戻した。"); }
                    break;
                }
            }
        }
