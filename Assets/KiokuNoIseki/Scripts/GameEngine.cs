...
namespace KiokuNoIseki
{
    ...
    public class GameEngine
    {
        ...
        public void BeginTurn()
        {
            ...
            phase = TurnPhase.Inscribe; // 生贄（任意）→そのまま行動可能に
        ...
        public bool Inscribe(CardInstance sacrificed)
        {
            ...
            Log($"{p.name}: 「{sacrificed.definition.trueName}」を捧げて生贄を行った（ゲージ上限+{up}）。");
            OnStateChanged?.Invoke();
            return true;
        }
        ...
        public void DoReincarnate(CardInstance card, RecallerState destroyer, RecallerState owner = null)
        {
            ...
            else
            {
                ...
                Log($"「{card.definition.trueName}」が刻印{card.engravingCount}を得てデッキへ転生。");
            }
        }
        ...
        public void EndTurn()
        {
            ...
            // 手札上限の超過処理：上限を超えた分はコストの低い順にデッキの下へ戻す
            while (p.hand.Count > p.HandLimit)
            {
                ...
                Log($"{p.name}: 手札上限({p.HandLimit})を超えたため「{discard.definition.trueName}」をデッキへ戻した。");
            }
            p.handLimitModifierThisTurn = 0;
